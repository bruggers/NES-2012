...
let gen_prop (alpha:alphabet) =
		let sub_alphabet = List.filter (fun x -> (Random.int 2)=1) alpha in
			...
			else parse_formula (lexer (Stream.of_string (
				fold_left (fun x y -> if (x="") then y
										else (if ((Random.int 2)=1) then x^"&"^y else x^"|"^y)) "" sub_alphabet)	
	))
	
	
let rec gen_previous (size:int) (alpha:alphabet) =
	if (size=0) then gen_prop alpha
	else Previous (gen_previous (size-1) alpha)
	
	

(* Generates randomly a future LTL formula - The generated formula may not be minimal and should be simplified before use. The boolean indicates whether the previous operator is allowed *)
let rec gen_1_form (size:int) (alpha:alphabet) (past:bool)=
	if size=1
		then
			gen_prop alpha
	else ( 
		if (past) then 
			match (Random.int 12) with
				...
				| 2 -> False
				| 3 -> gen_prop (alpha)
				| 4 -> Or (gen_1_form (size-1) alpha past , gen_1_form(size-1) alpha past)
				| 5 -> And (gen_1_form(size-1) alpha past, gen_1_form(size-1) alpha past)
				| 6 -> Neg (gen_1_form(size-1) alpha past)
				| 7 -> Iff (gen_1_form(size-1) alpha past,gen_1_form(size-1) alpha past)
				| 8 -> Until (gen_1_form(size-1) alpha past,gen_1_form(size-1) alpha past)
				| 9 -> Next (gen_1_form(size-1) alpha past)
				| 10 -> Glob (gen_1_form(size-1) alpha past)
				| 11 -> Ev (gen_1_form(size-1) alpha past)
				| 12 -> Previous (gen_previous (size - 1) alpha)
				| _ -> False
			else
				match (1+Random.int 9) with
   				1 -> True
				| 2 -> False
				| 3 -> gen_prop (alpha)
				| 4 -> Or (gen_1_form (size-1) alpha past , gen_1_form(size-1) alpha past)
...
let rec gen_1_form_nontrivial (size:int) (alpha:alphabet) (past:bool)=
	let gen (size:int) (alpha:alphabet) (past:bool) =
	if (size=0)
		then
			gen_prop alpha
		else (
			...
			match (Random.int 9) with
				| 0 -> gen_prop (alpha)
				| 1 -> Or (gen_1_form_nontrivial (size-1) alpha past , gen_1_form_nontrivial(size-1) alpha past)
				| 2 -> And (gen_1_form_nontrivial(size-1) alpha past, gen_1_form_nontrivial(size-1) alpha past)
				| 3 -> Neg (gen_1_form_nontrivial(size-1) alpha past)
				| 4 -> Iff (gen_1_form_nontrivial(size-1) alpha past,gen_1_form_nontrivial(size-1) alpha past)
				| 5 -> Until (gen_1_form_nontrivial(size-1) alpha past,gen_1_form_nontrivial(size-1) alpha past)
				| 6 -> Next (gen_1_form_nontrivial(size-1) alpha past)
				| 7 -> Glob (gen_1_form_nontrivial(size-1) alpha past)
				| 8 -> Ev (gen_1_form_nontrivial(size-1) alpha past)
				| 9 -> Previous (gen_previous (size - 1) alpha)
				| _ -> gen_prop (alpha)
			else
		...
		in let form = simp (gen size alpha past) in
			if (form= True || form= False) then gen_1_form_nontrivial size alpha past
				else form
	
let rec generate_array_formulae_same (num:int) (size_max:int) (alpha: alphabet) (past:bool) =
	if (num=0) then raise Error
	else
		let form= (gen_1_form (Random.int size_max) alpha past) in
			Array.make num form
